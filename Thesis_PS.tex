...
We observed that, when oscillating input were given, output firing rate is higher than that of static input even though the overall firing rate of both input were same. Also, the specific level of response can be made with less strength of connection in oscillating input compared to the static input. We showed that the oscillating input induced more output spike than static input. In addition, the gain of response from static to oscillating input are largest in UU rule compared to GG and UE rules. We discovered that the output spikes in UU rule have the most dependency on synchronization level in input compared to GG and UE rules. These results showed that UU has the most sensitivity on the synchronization level in input.
